...
# ─── NOUVEAU : Génération template vierge par mois ───────────────────────────
def generer_template_mois(template_path, output_path, annee, mois):
    """
    Génère un template d'affichage vierge pour un mois donné à partir
    du template de référence (template_vierge_SEPTEMBRE_2026.xlsx).

    Règles :
    - 1 jour = 6 lignes dans le template
    - Si le mois commence un Jeudi → supprimer Lun+Mar+Mer = 18 lignes
    - Si le mois commence un Mardi → supprimer Lun = 6 lignes
    - etc.
    - Corrige le bug openpyxl : delete_rows ne décale pas les hauteurs
      ni ne reporte certaines fusions → on sauvegarde et on restaure
    - Met à jour les labels de jours sur les 4 colonnes : 2, 26, 54, 76
    """

    # Colonnes contenant les labels de jours (découvertes par analyse du template)
    ...
    # ── Calculer les lignes à supprimer en début ──────────────────────────────
    # Trouver le premier jour ouvré réel du mois
    premier = datetime.date(annee, mois, 1)
    while premier.weekday() >= 5 or premier in FERIES:
        premier += datetime.timedelta(days=1)

    # Lundi=0 → 0 lignes, Mardi=1 → 6 lignes, ..., Vendredi=4 → 24 lignes
    ...
    # ── Supprimer les lignes manquantes + corriger le bug openpyxl ────────────
    if lignes_a_supprimer > 0:
        DELETE_START = 7
        DELETE_COUNT = lignes_a_supprimer
        FIRST_KEPT   = DELETE_START + DELETE_COUNT

        # Sauvegarder les hauteurs AVANT suppression
        saved_heights = {
        ...
        # Ré-appliquer les hauteurs correctes (décalées de -DELETE_COUNT)
        ...
        # Ré-appliquer les fusions manquantes
        # (delete_rows d'openpyxl laisse des trous dans les fusions
        #  sur les lignes qui avaient une hauteur de séparateur)
        current_merges = {
        ...
        for r in range(DELETE_START, ws.max_row + 1):
            ...
            if h is None or h >= 10:  # ignorer les séparateurs (h ≈ 6.6)
    ...
    # ── Mettre à jour les labels de jours (toutes les colonnes) ──────────────
    # Détecter dynamiquement les positions après suppression éventuelle
    jours_pos = []
